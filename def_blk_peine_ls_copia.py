...
def sim(SNRdB, per_peine=8, per_bloque=20, hab_peine=True, variante=False):
    N = 128 # numero de subportadoras
    pilot_period_comb = per_peine # un piloto cada esta cantidad de simbolos
    cant_pilotos_comb = N // pilot_period_comb 
    pilot_period_blk = per_bloque #pilotos bloque
    ...
    for idx in range(0,rx_symb.shape[1]):
        ...
        if variante:
            ...
            H = (Hgain)*H + 0.001*Hnoise
        else:
            # Tambien es variante, pero varia muchisimo menos
            H = 0.998*H + 0.001*channels.fadding_channel(N)
